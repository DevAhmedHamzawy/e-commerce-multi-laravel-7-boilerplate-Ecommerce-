...
class CheckOutController extends Controller
{
    ...
    {
        return view('main.checkout.shipping_address', ['cities' => Area::whereParentId(15)->get()]);
    }

    public function storeShippingAddress(Request $request)
    {

        if($request->deliver_another_person == 1) { 
            $ch = new CheckOutController;
            return $ch->storeTheShippingAddress($request);
        }
    ...
    public function storeTheShippingAddress(Request $request)
    {
        ...
        $shippingAddress = ShippingAddress::create($request->except('deliver_another_person'));
