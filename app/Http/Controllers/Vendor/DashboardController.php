...
class DashboardController extends Controller
{
    ...
    public function index()
    {
       ...
       $ProductsInYearChart->height(200);
          ...
          $postsByMonthChartTwo->dataset('My dataset', 'line', $postsInMonth)->backgroundColor("rgba(45, 52, 54, 0.7)");
    ...
    public function sendVerification()
    {
        $details = [
            ...
            'link' => URL::route('verify_store', auth()->user()->id), 
        ];

        Mail::to(auth()->user()->email)->send(new \App\Mail\VendorVerification($details));

        return redirect()->back()->withResent(1);
    }

    public function verifyStore($id)
    {
        $store = Admin::findOrFail($id);

        $store->update(['verified' => 1]);

        $details = [
            'title' => 'منصة مزارعنا',
            'body' => 'مرحبا بك فى منصة مزارعنا',
            'link' => URL::route('v_dashboard'), 
        ];
