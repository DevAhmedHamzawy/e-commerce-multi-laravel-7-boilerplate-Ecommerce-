...
/*
|--------------------------------------------------------------------------
| Web Routes (NOTE:- SPECIALIZED FOR ADMIN AND VENDOR ROUTES)
|--------------------------------------------------------------------------
|
...
Route::group(['middleware' => ['assign.guard:admin,admin/login', 'role:0'] , 'prefix' => 'admin'],function(){
    ...
    Route::resource('producers_categories', 'Admin\CategoryOneController');
    Route::get('producers_categories/{producers_category}/delete', 'Admin\CategoryOneController@destroy')->name('producers_categories.delete');

    Route::resource('stores_categories', 'Admin\CategoryTwoController');
    Route::get('stores_categories/{stores_category}/delete', 'Admin\CategoryTwoController@destroy')->name('stores_categories.delete');
    ...
    Route::get('places/{place}/delete', 'Admin\PlaceController@destroy')->name('places.delete');
    

    // Cities
    Route::resource('places/{place}/cities', 'Admin\CityController');  
    Route::get('places/{place}/cities/{city}/delete', 'Admin\CityController@destroy')->name('cities.delete');

    Route::resource('a_product_messages', 'Admin\MessageController')->only(['index','show','store']);
...
Route::group(['middleware' => ['assign.guard:admin,admin/login', 'role:1'], 'prefix' => 'vendor'],function(){
    ...
    Route::get('vendor_verify', 'Vendor\DashboardController@getVerificationPage')->name('vendor_verify');

    Route::post('vendor_verify_send', 'Vendor\DashboardController@sendVerification')->name('vendor_verify_send');

    Route::get('verify_store/{id}', 'Vendor\DashboardController@verifyStore')->name('verify_store');
    ...
    Route::post('v_products/deletemany' , 'Vendor\ProductController@deleteMany')->name('v_products.delete_many');
    ...
    Route::resource('v_product_messages', 'Vendor\MessageController')->only(['index','show','store']);

    Route::resource('services', 'Vendor\ServiceController');
    Route::get('services/{service}/delete', 'Vendor\ServiceController@destroy')->name('services.delete');
