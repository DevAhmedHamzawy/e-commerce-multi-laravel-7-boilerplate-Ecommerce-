      ...
      <form method="POST" action="{{ auth()->user() ? route('save_shipping_address') : route('save_the_shipping_address') }}">
        ...
        <select name="city_id" class="fadeIn third textinput" required>
           ...
           @foreach ($cities as $city)
               <option value="{{ $city->id }}">{{ $city->name }} - {{ trans('order.shipping_cost') }} :- {{ $city->shipping_cost }}</option>
           @endforeach
        </select>
        @error('city_id')
            <span class="invalid-feedback" role="alert" style="display: block">
        ...
        <div class="form-group row">
            <div class="col-md-6 offset-md-4">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="deliver_another_person" id="deliver_another_person" {{ old('deliver_another_person') ? 'checked' : '' }} value="1">

                    <label class="form-check-label" for="deliver_another_person">
                        توصيل لشخص اخر
                    </label>
                </div>
            </div>
